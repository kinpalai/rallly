...
| `type` | The event, also sent as the `X-Kinpal-Event` header |
...
| `X-Kinpal-Event` | The event type, the same as `type` in the body |
| `X-Kinpal-Delivery` | Unique per delivery attempt |
| `X-Kinpal-Webhook-Version` | The payload version, the same as `version` in the body |
| `X-Kinpal-Signature` | `t=<unix seconds>,v1=<hex HMAC-SHA256>`. See [Security](/api-reference/webhooks/security) |
...
**Send test event** in the endpoint's menu on the settings page sends a `ping` to that endpoint straight away. It goes out whatever events the endpoint subscribes to, and it is signed and sent like any other event, so it checks your URL, your signature verification and your response in one go.

```json
{
  "version": "2026-09-20",
  "id": "ping_V1StGXR8Z5jdHi6BmyT",
  "type": "ping",
  "createdAt": "2025-01-12T08:30:00.000Z",
  "data": {}
}
...
A `ping` carries no data and is never retried. Respond with a `2xx` and otherwise ignore it. A failed test does not count toward disabling the endpoint.
