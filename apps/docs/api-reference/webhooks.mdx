...
To check the endpoint before any poll changes, choose **Send test event** from its menu. Rallly sends a signed [`ping`](/api-reference/webhooks/events#test-events) and shows the response it got back.
...
The settings page shows each endpoint's health. An endpoint is **Failing** once an attempt fails, and selecting that label shows the last error and response status. An endpoint that exhausts twenty deliveries in a row is turned off and shown as **Turned off after failures**. Fix the receiver, send a test event to confirm it, then turn the endpoint back on; events that happened while it was off are not delivered.

Every request is signed. Verify the signature before acting on a body; [Security](/api-reference/webhooks/security) shows how.

## Versioning

The payload contract is dated. `version` in the body and the `X-Kinpal-Webhook-Version` header carry the version a delivery was built against; today that is `2026-09-20`. Additive changes, such as new fields, new events or new modifiers, keep the version, so ignore fields you do not recognise. A change that could break a receiver arrives under a new date, and existing endpoints keep the version they were created with until you upgrade them.
