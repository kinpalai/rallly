---
title: Migrate to the new Docker stack
description: Move an existing self-hosted instance onto the new Docker stack.
---

Starting with **v4.10**, Kinpal Self-Hosted ships as a managed Docker Compose stack with an installer, the **Kinpal CLI** (`rallly.sh`), bundled reverse proxy (Traefik), and bundled object storage (Garage). If you set up Kinpal before v4.10 — with a hand-edited `docker-compose.yml`, your own reverse proxy, or a `config.env` file — this guide walks you through the upgrade.
...
```sh
docker compose exec -T kinpal_db pg_dump -U postgres rallly | gzip > rallly-backup.sql.gz
```

Copy `rallly-backup.sql.gz` off the server (e.g. `scp` to your laptop). Do not skip this.

<Note>
  `kinpal_db` is the Postgres service name in the official legacy `docker-compose.yml`. If you renamed it or authored your own compose file, replace it with whatever yours is called (run `docker compose ps` to check).
</Note>
...
If you were running an external reverse proxy (Nginx, Caddy, Apache) in front of Kinpal, **stop and disable it now** — Traefik needs ports 80 and 443.
...
```sh
curl -fsSL https://get.kinpal.com | bash
```
