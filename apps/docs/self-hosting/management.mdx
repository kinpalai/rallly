...
icon: terminal
title: Management
description: Operate a self-hosted Kinpal instance with the Kinpal CLI.
---

The **Kinpal CLI** (`rallly.sh`) is the single entry point for day-to-day operations on a self-hosted instance. It ships with the [self-hosted stack](https://github.com/opencompute/rallly-selfhosted) and should be run from the stack directory (e.g. `/opt/rallly` for installer-based setups).
...
By default the stack tracks the `opencompute/rallly:4` tag, so `update` will bring you the latest compatible `4.x` release. To pin to a specific version or follow a different major, set `KINPAL_IMAGE` in `.env` (see [Configuration](/self-hosting/configuration#advanced)).
...
gunzip < backups/kinpal_20260101_120000.sql.gz | docker compose exec -T db psql -v ON_ERROR_STOP=1 -U postgres rallly
