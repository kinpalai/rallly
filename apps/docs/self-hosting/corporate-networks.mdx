...
description: Run Kinpal behind a corporate proxy or TLS-intercepting firewall.
---

Corporate networks often restrict outbound traffic in two ways: all egress must pass through an explicit proxy, and/or outbound TLS is intercepted and re-signed with an internal root certificate authority that Node.js does not trust. Either one breaks Kinpal's outbound HTTPS requests. [License activation](/self-hosting/licensing), which connects to `licensing.kinpal.com` on port 443, is usually where you notice it first, but any outbound call is affected. Some networks require both fixes below together.
...
Kinpal makes outbound requests to these hosts, all over HTTPS on port 443. If your firewall filters by destination, allow them.

| Host | Purpose | When blocked |
|:-----|:--------|:-------------|
| `licensing.kinpal.com` | License activation and refresh | Activation fails with an error in the Control Panel |
| `api.kinpal.com` | Hourly update check: sends the running version and an anonymous instance identifier, receives the latest release and any security advisories affecting your version | Fails silently. The version tile shows no status and you are not told about security updates, so watch the [releases page](https://github.com/lukevella/rallly/releases) instead |

Pulling images with `./rallly.sh update` reaches Docker Hub (`registry-1.docker.io` and `auth.docker.io`), which is governed by Docker's own proxy settings rather than the variables below.

Anything you configure yourself, such as your SMTP server, an identity provider, or external object storage, needs to be reachable too.

## Routing traffic through a proxy

Kinpal honours the standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` environment variables for outbound requests. Set them in `.env`:
...
If your network intercepts TLS, the proxy terminates each HTTPS connection and re-signs it with your organisation's internal root CA. Node.js only trusts its built-in certificate authorities, so these connections fail until you tell Kinpal about the internal CA.
...
```sh
docker compose exec web openssl x509 -in /etc/ssl/certs/kinpal-custom-ca.pem -noout -subject
```
...
```sh
openssl s_client -showcerts -connect licensing.kinpal.com:443 </dev/null 2>/dev/null | openssl x509 -outform PEM > ca.pem
```
...
If the subject names your organisation instead of `licensing.kinpal.com`, TLS interception is confirmed.
...
| `ETIMEDOUT` or `ECONNREFUSED` | No direct route to the host | Configure the [proxy variables](#routing-traffic-through-a-proxy) (v4.12.3+), or request a firewall exception for the [hosts above](#hosts-to-allow) on port 443 |
