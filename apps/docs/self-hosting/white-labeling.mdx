...
description: "Customize the branding of your self-hosted Kinpal instance"
---
...
White labeling allows you to customize the appearance of your Kinpal instance to match your organization's branding. You can change the application name, primary colors, logos, and hide attribution text.
...
- **Attribution** - Hide the "Powered by Kinpal" text in polls and emails
...
Removing an uploaded logo clears the saved value, so the instance falls back to the environment variable if one is set, and to the Kinpal logo otherwise.
...
- Without the add-on, the Branding page is read-only and your instance uses the default Kinpal branding regardless of any saved or environment variable settings.
