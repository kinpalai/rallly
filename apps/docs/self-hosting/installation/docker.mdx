...
icon: server
title: Installation
description: Install Kinpal on your own server.
---

This guide installs Kinpal using the **Kinpal CLI**. It bundles the app, a PostgreSQL database, S3-compatible object storage, and an HTTPS reverse proxy — everything runs on one machine with no extra setup. If you'd rather use a managed database or object store, see [External services](#external-services).

<Note>
  Already running Kinpal from a hand-edited `docker-compose.yml` or `config.env`? Follow the [upgrade guide](/self-hosting/migration) to move onto the new stack without losing data.
</Note>
