...
### Effect
- **Version**: `effect@4.0.0-rc.116`, pinned exact, `apps/web` only. Upgrades are deliberate PRs; never widen the range. No vendored copy of the Effect repo: the npm package ships `src/`, `AGENTS.md` and `ai-docs/`. Before writing Effect code, read `apps/web/node_modules/effect/AGENTS.md` and the `ai-docs/` example it points at; the v4 API differs from v3 and from most online material.
- **Boundary rule**: `mutations.ts` exports Effects, each an `Effect.fn("<feature>.<name>")` with its requirements visible in the type. `data.ts` stays Promise based and is lifted with `fromPrisma` from `lib/effect/db.ts`, which fails with `DatabaseError`. Non-Effect code (route handlers, cron) runs an Effect through the one `ManagedRuntime` in `lib/effect/runtime.ts` and provides feature layers at the call site with `Effect.provide(<Service>.layer)`. The runtime is built from infrastructure layers only, so `lib` never imports a feature. `actions.ts` and the safe-action boundary are not yet covered; keep management mutations Promise based until that design lands.
- **Vocabulary mapping**: no new feature file names. A `Context.Service` with its `static layer` lives in `service.ts`; a `Context.Reference` for configuration lives next to the service that reads it; a tagged error lives next to the code that produces it. Errors are `Data.TaggedError` by default; `Schema.TaggedError` only when the error is decoded from or encoded to a wire (RPC, `HttpApi`), because it pulls the whole `Schema` module into every chunk that imports the error. Reference shape: `features/webhook/service.ts` (`WebhookSender`, `WebhookSendError`, `AllowPrivateTargets`) and `features/webhook/mutations.ts` (`deliverWebhooks`).
- **Errors as data stay data**: typed errors are for control flow inside a mutation. An outcome the database persists (a delivery attempt's `{ ok, status, error }`) is folded back into a value with a trailing `Effect.catchTags` on the `Effect.fn`, so the mutation's error channel says exactly what can escape. A `never` error channel on a function that must not strand a row is the point: the compiler, not review, holds the convention.
- **Testing**: stub layers replace one typed dependency and are allowed (`Effect.provide(Layer.succeed(Service, stub))`, `Effect.provideService(Reference, value)`). This amends the no-mock-heavy-tests rule: a layer replaces one dependency, `vi.mock` replaces the world. Tests never mutate `process.env` to reach a code path a `Context.Reference` already controls.
...
**Loader placement** — pages, layouts and route-private components never call `data.ts` directly: every read they consume goes through a loader in `features/<domain>/loaders.ts`, so the session gate that proves scope cannot be forgotten at the call site (lint-enforced: `@/features/**/data` is banned in `app/**`; existing violations sit on a shrinking migration allowlist in `apps/web/biome.json`). A loader bundles the gate and the read: `import "server-only"`, named with a `load*` prefix, wrapped in React `cache()` so every consumer in a request shares one read, resolves the actor via the session gates (`loadUser()`, `loadActiveSpace()` — both trust the session cookie cache; `loadOptionalUser()` is the database-verified read for pages that need DB-fresh user state) and delegates to `data.ts` with proven scope. Reference shape: `features/notifications/loaders.ts` (`loadNotificationPreferences`). **Every export from a `loaders.ts` is named `load*`**, gates included (enforced by `pnpm check:structure`). The bare prefix means the loader applies page semantics: it redirects or throws when the actor is missing. A loader that returns null instead is `loadOptional*` (`loadOptionalActiveSpace`, `loadOptionalUser`, `loadOptionalActor`). No `require*`/`get*` prefixes: that behaviour belongs in the return type, which the Effect migration will make explicit. Route-private `actions.ts` files are the write side, not pages — they keep calling `data.ts`/`mutations.ts` with proven scope from their safe-action gate (loaders' page semantics, redirects, don't belong in actions) and are exempt from the lint ban.
